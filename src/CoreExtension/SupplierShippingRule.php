...
class SupplierShippingRule extends Data implements
    ...
    Data\CustomVersionMarshalInterface
{
     ...
     * @var string
     */
    public $fieldtype = 'coreShopMarketWarehouseSupplierShippingRule';
    ...
    {
        Assert::isInstanceOf($object, SupplierCarrierInterface::class);
    ...
    public function getDataForEditmode($data, $object = null, $params = [])
    {
        ...
        if ($object instanceof SupplierCarrierInterface) {
    ...
    public function getDataFromEditmode($data, $object = null, $params = [])
    {
        ...
        if ($data && $object instanceof Concrete) {
            foreach ($data as $dataRow) {
                ...
                $form = $this->getFormFactory()->createNamed('', SupplierShippingRuleType::class, $storedRule);

                $form->submit($dataRow);

                if ($form->isValid()) {
                    $formData = $form->getData();
                    $formData->setSupplierCarrier($object->getId());
        ...
        if ($object instanceof SupplierCarrierInterface) {
            ...
            if (is_array($existingPriceRules)) {
                foreach ($existingPriceRules as $price) {
                    if ($price instanceof SupplierShippingRuleInterface) {
                        $entityMerger->merge($price);

                        $price->setSupplierCarrier($object->getId());
    ...
    public function load($object, $params = [])
    {
        if (isset($params['force']) && $params['force']) {
            return $this->getRepository()->findForSupplierCarrier($object);
        }
